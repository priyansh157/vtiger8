...
class CustomField_createMultiSelectField_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		$pickListVal = $request->get('pickListVal');
		
		$plvExp = explode(",",$pickListVal);
		$plValue = array();
		for($pvc=0;$pvc<count($plvExp);$pvc++){
			if(!empty($plvExp[$pvc])){
				array_push($plValue,trim($plvExp[$pvc]));
			}
		}
		...
		$fieldInstance->setPicklistValues($plValue);
