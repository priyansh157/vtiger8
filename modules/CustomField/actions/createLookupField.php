...
class CustomField_createLookupField_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		if($fieldResult){
			$sqlFD = "SELECT vtiger_field.fieldid FROM vtiger_field WHERE vtiger_field.fieldname=? AND vtiger_field.tablename=?";
			$dataFD = array(str_replace(" ","_",strtolower($fieldName)),$moduleTable);
			$resFD = $adb->pquery($sqlFD,$dataFD);
			$detFD = array();
			$detFD = $adb->fetchByAssoc($resFD);
			if(!empty($detFD['fieldid'])){
				$sqlFR = "INSERT INTO vtiger_fieldmodulerel SET fieldid=?,module=?,relmodule=?";
				$dataFR = array($detFD['fieldid'],$selectModule,$targetModule);
				$resFR = $adb->pquery($sqlFR,$dataFR);
			}
			echo 'Field created';
		}else{
